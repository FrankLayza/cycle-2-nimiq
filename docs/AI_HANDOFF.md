...
- **UI verification:** current typecheck, client tests, and client production build pass. The build is split into ~104 kB initial JS, ~327 kB gzip on-demand Phaser chunk, and ~4 kB Today's Run view chunk. Browser screenshots via Playwright/Chromium **are** available in this environment (see the D46/D47 section); a real-device Nimiq Pay pass for safe areas and rotation is still required.
...
Committed in `9edcde1`, `6b65974`, `8f8d3d3`, `5ca83d7`, `0a2bb3c`, `3e649f3`.
...
- **Snakes (D47):** generated, not sourced. `game/snakeSprites.ts` rasterises head/straight/corner/tail as a distance field around a centreline polyline, so a corner is a *bent* line and its elbow rounds — which is what answers "the snakes just turn at 90 degrees". Movement stays grid-locked; the sim is integer-only and must remain so. `game/snakeTexture.ts` bakes those masks into one canvas atlas (4 seats x 4 pieces) at scene create; sprites are pooled and each piece draws twice, once as a dark offset silhouette. Two asset packs were tried first and neither fit: top-down grid snakes need a per-direction head plus corner elbows, and the available sheets were side/front-view characters with a shadow baked along one edge and three fixed saturated hues.
- **Lobby:** the turf is a *contained* arena-preview panel (an 809-byte inlined patch, not a live Phaser instance). Tiling it full-bleed first turned the texture into confetti wallpaper. The marketing-shaped hero, the symmetric stat grid and its stale "1v1" claim, and the CSS faux-3D snakes are gone. `PixelIcon` draws icons as rects on the same 16px lattice; interface emoji are removed (kept only in outbound share text).
...
### Verified in a browser, and what that pass found

Automated screenshots **are** available here: Playwright plus the bundled Chromium, driven against `pnpm --filter @snake/client dev`. Earlier handoffs said otherwise. Use it — every item below was invisible to typecheck, tests, lint and the build.

- **Viewport media queries are the wrong tool under rotation.** Tailwind's `sm:`/`lg:` and `@media (orientation: portrait)` describe the *device*: a rotated phone reports a 390px portrait viewport while the stage is 844px landscape. The lobby took its narrow branch on a wide stage, and the match HUD stacked its rails precisely when they should have sat in the margins. `.landscape-stage` is now a query container (`container-type: inline-size`) so Tailwind's `@` container variants measure the stage, and it publishes `data-orientation` for the one thing a width query cannot express — whether the stage is wider than it is tall. **Use `@` variants, not `sm:`/`lg:`, for anything inside the stage.**
- **Safe-area insets are remapped, no longer a pending risk.** The rotation sends the stage's local top to the screen's right, so `env(safe-area-inset-*)` guarded the wrong edges. `.landscape-stage-rotated` now permutes them to follow the transform.
- **Turf decoration must not look collectible.** The Kenney flower tile at cell size is an orange mark on green that reads as a pickup; the field was scattered with them, competing with the two real pellet types. Ground cover is plain grass and tufts only.
- **Sprite outlines need to be near-ink, not a darker tint.** The teal seat sits close to the turf hue and its body dissolved into the field until `outlineOf()` was introduced.

**Still not run on a real device.** A physical Nimiq Pay WebView pass remains outstanding — notably whether the CSS rotation and the remapped insets behave on a notched phone.
