...
export function registerRooms(app: FastifyInstance): void {
  app.post('/api/v1/rooms', async (req, reply) => {
    ...
    ).run(code, mode, code, '2', now, now + ROOM_TTL_MS);
    return { code, mode, roomId: code, wsUrl: '/colyseus', expiresAt: now + ROOM_TTL_MS };
  });
  ...
  app.get<{ Params: { code: string } }>('/api/v1/rooms/:code', async (req, reply) => {
    ...
    const row = getDb()
      .prepare('SELECT code, mode, expires_at FROM rooms WHERE code = ?')
      .get(code) as { code: string; mode: string; expires_at: number } | undefined;
    if (!row || row.expires_at < Date.now()) return reply.code(404).send({ error: 'room not found or expired' });
    return { code: row.code, mode: row.mode, roomId: row.code, wsUrl: '/colyseus', expiresAt: row.expires_at };
  });
}
